...
# Candidate-retrieval caps per source. Scaled up for "full" so that, combined
# with the ~70% actionable-complaint rate observed empirically, the resulting
# rank_pairs.csv lands in the spec's 5-20 million row target without wasting
# rows on a retrieval strategy that doesn't need them (spec section 2/21).
CAND_CAPS = {
    "demo": dict(geo=60, hotspot=40, network=40, behav=40),
    "dev": dict(geo=90, hotspot=60, network=60, behav=60),
    "full": dict(geo=120, hotspot=90, network=90, behav=90),
}
CAND_N_GEO = CAND_CAPS[SCALE]["geo"]
CAND_N_HOTSPOT = CAND_CAPS[SCALE]["hotspot"]
CAND_N_NETWORK = CAND_CAPS[SCALE]["network"]
CAND_N_BEHAV = CAND_CAPS[SCALE]["behav"]
