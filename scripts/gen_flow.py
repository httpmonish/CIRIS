...
def generate_flow_for_complaints(rng, complaints_df, accounts_df, atms_df, clusters, cluster_assign):
    ...
    for i, row in enumerate(complaints_df.itertuples(index=False)):
        ...
        # Chain length 2-4 hops through cluster members (no immediate repeats,
        # so no from==to self-loop transactions)
        hop_count = min(rng.integers(2, 5), max(1, len(members)))
        chain = []
        last = victim_acc
        pool = list(members)
        ...
        # decide fragmentation vs single-shot
        frag = rng.random() < 0.4

        for hop_i in range(len(full_chain) - 1):
            frm, to = full_chain[hop_i], full_chain[hop_i + 1]
            n_splits = rng.integers(2, 5) if (frag and hop_i == 0) else 1
